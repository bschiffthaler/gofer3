                         ...
                         std::unordered_set<std::string>& test_set)
{
  ...
  if (type == "go")
  {
    ...
    for (const std::string& id : test_set)
    {
      ...
      size_t tindex = 0;
      if (e.mapping_gos[t].in_a(id)) // Look only in specific matches
      {
        for (const std::string b : e.mapping_gos[t].a_to_b(id))
        {
  ...
  else if (type == "hierarchical")
  {
    ...
    for (const std::string& id : test_set)
    {
      ...
      ret[t][index]["terms"] = web::json::value::array();
      size_t tindex = 0;
      if (e.mapping_hierarchicals[t].in_a(id)) // Look only in specific matches
      {
        for (const std::string b : e.mapping_hierarchicals[t].a_to_b(id))
        {
                         ...
                         const web::json::value json)
{
  ...
  if (type == "go")
  {
    ...
    for (const std::string& term : term_set)
    {
      ...
      size_t tindex = 0;
      if (e.mapping_gos[name].in_b(term))
      {
        for (const std::string a : e.mapping_gos[name].b_to_a(term))
        {
  ...
  else if (type == "hierarchical")
  {
    ...
    for (const std::string& term : term_set)
    {
      ...
      ret[name][index]["ids"] = web::json::value::array();
      size_t tindex = 0;
      if (e.mapping_hierarchicals[name].in_b(term))
      {
        for (const std::string a : e.mapping_hierarchicals[name].b_to_a(term))
        {
          ret[name][index]["ids"][tindex] = web::json::value::string(a);
          tindex++;
        }
