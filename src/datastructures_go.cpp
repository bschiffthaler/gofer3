...
void go::parse(go_opt & opts) {
  ...
  _hash = std::hash<std::string> {} (all_data.str());
  log(LOG_INFO) << "File hash: " << std::hex << _hash << "\n" << std::dec;
}
...
void mapping_go::add(std::unordered_map<std::string,
...
mapping_go::make_background(std::unordered_set<std::string>& gene_set) {
  ...
  {
    auto query = _a_to_b_spec.find(it);
    if (query == _a_to_b_spec.end())
      not_found++;
    else
    {
      ret.add(query);
...
mapping_go::all_a(std::unordered_set<std::string>& b)
{
  std::unordered_set<std::string> ret;
...
std::unordered_set<std::string>
mapping_go::all_b(std::unordered_set<std::string>& a)
{
  std::unordered_set<std::string> ret;
